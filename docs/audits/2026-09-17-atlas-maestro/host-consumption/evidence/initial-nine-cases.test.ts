...
const cases = ["fresh-tool", "dirty-cold-tool", "dirty-warm-tool", "committed-cold-tool", "deleted-cold-tool", "fresh-command", "dirty-command", "missing-tool", "ordinary-skill"] as const
...
for (const mode of cases) {
  it.instance(mode, () => Effect.gen(function* () {
    ...
    if (mode.startsWith("dirty") || mode.startsWith("committed")) writeFileSync(src, "export const value = 2;\n")
    ...
    const verification = artifacts.verifyStaticOwnFreshness(artifact, currentBlob)
    ...
    const tools = JSON.stringify(hits[0]?.body.tools)
    ...
    records.push({ mode, verified: verification, agent: selected.id, requestCount: hits.length,
      endpoint: last.url.pathname, hasMaestroSystem: messages.includes("You are Maestro"),
