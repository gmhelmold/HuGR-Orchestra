...
<Warning>
  Setting `output.continue = true` re-runs the same `lastUser` through the LLM with no synthetic prompt. Use sparingly —
  the loop will not re-ask the user for input. To break out of a continued loop, your plugin must not set
...
const plugin: Hooks = {
  "session.end": async (input) => {
    console.log(`session ${input.sessionID} ended after ${input.duration_ms}ms (${input.reason})`)
  },
}
