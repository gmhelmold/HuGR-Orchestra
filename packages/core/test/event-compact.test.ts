...
describe("EventV2.compactSnapshotEvents", () => {
  it.effect("keeps only the latest message.updated and part.updated per entity", () =>
    Effect.gen(function* () {
      ...
      yield* db
        ...
        .values([{ aggregate_id: "ses_a", seq: 10 }])
        .run()
        .pipe(Effect.orDie)
      yield* insert(db)([
        {
  ...
  it.effect("removes nothing when no duplicate snapshots exist", () =>
    Effect.gen(function* () {
      ...
      yield* db
        .insert(EventSequenceTable)
        .values([{ aggregate_id: "ses_b", seq: 1 }])
        .run()
        .pipe(Effect.orDie)
      yield* insert(db)([
        {
  ...
  it.effect("detects a pre-existing sequence gap", () =>
    Effect.gen(function* () {
      const { db } = yield* Database.Service
      yield* db.insert(EventSequenceTable).values([{ aggregate_id: "ses_c", seq: 3 }]).run().pipe(Effect.orDie)
      yield* insert(db)([
        { id: "e1", aggregateID: "ses_c", seq: 1, type: "session.created.1", data: { sessionID: "ses_c" } },
        { id: "e3", aggregateID: "ses_c", seq: 3, type: "session.updated.1", data: { sessionID: "ses_c" } },
      ])
