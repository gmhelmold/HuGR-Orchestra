...
export const materialize = Effect.fn("ConfigReference.materialize")(function* () {
  const config = yield* Config.Service
  const location = yield* Location.Service
  const global = yield* Global.Service
  const reference = yield* Reference.Service
  yield* reference.transform(
...
export const node = makeLocationNode({
  name: "config-reference-materialization",
  layer: Layer.effectDiscard(materialize()),
  deps: [Config.node, Reference.node, Global.node, Location.node],
})
